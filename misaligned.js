...
function print_color_map() {
    ...
    for (let i = 0; i < majorColors.length; i++) {
        for (let j = 0; j < minorColors.length; j++) {
            ...
            const paddedMajorColor = majorColors[i].padEnd(maxMajorColorsWidth); // Padding major colors for consistent width
            const paddedMinorColor = minorColors[j].padEnd(maxMinorColorsWidth); // Padding minor colors for consistent width
